...
bool hesai_ros_driver__msg__ptp__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // ptp_status_size
    PyObject * field = PyObject_GetAttrString(_pymsg, "ptp_status_size");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->ptp_status_size = PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
  {  // ptp_status
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      if (!rosidl_runtime_c__uint8__Sequence__init(&(ros_message->ptp_status), size)) {
        PyErr_SetString(PyExc_RuntimeError, "unable to create uint8__Sequence ros_message");
        PyBuffer_Release(&view);
        Py_DECREF(field);
        return false;
      }
      uint8_t * dest = ros_message->ptp_status.data;
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'ptp_status'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
PyObject * hesai_ros_driver__msg__ptp__convert_to_py(void * raw_ros_message)
{
  ...
  {  // ptp_status_size
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->ptp_status_size);
    {
      int rc = PyObject_SetAttrString(_pymessage, "ptp_status_size", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
  ...
  {  // ptp_status
    ...
    if (ros_message->ptp_status.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      uint8_t * src = &(ros_message->ptp_status.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->ptp_status.size * sizeof(uint8_t));
      assert(data != NULL);
